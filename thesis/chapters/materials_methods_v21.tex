...
\begin{enumerate}
    ...
    \item \textbf{Spectral Extraction:} The tool extracted the 204-band spectral vectors for all pixels within the projected mask. The user assigned a definitive class label to the region, and the grouped signatures were stored.
\end{enumerate}
...
\clearpage
\subsection{Whole-Image Dataset and Spatial Partitioning}
...
\medskip
\noindent
To ensure rigorous evaluation and prevent spatial autocorrelation or information leakage, a strict row-based spatial partitioning scheme was enforced. All records acquired from vineyard **Row 1** were designated as the calibration and training set, while records from **Row 2** served exclusively as the held-out evaluation (test) set. Because the two rows are physically disjoint, this split robustly evaluates the model's ability to generalize to unseen vines.

\medskip
\noindent
Furthermore, whole-image evaluations were stratified by the physiological stage of the disorder at the time of measurement:
\begin{itemize}
    \item \textbf{Early-stage:} Images representing the initial onset of cracking for a specific cluster (the earliest date the anomaly was observed).
    \item \textbf{Late-stage:} Images acquired in subsequent weeks, exhibiting advanced, macroscopically visible cracking.
\end{itemize}

\medskip
\noindent
Crucially, to simulate a realistic early-warning scenario, the underlying pixel-level models were trained \textit{exclusively} using pixels extracted from Late-stage images in Row 1. Early-stage records were entirely excluded from pixel-level training. The whole-image aggregation hyperparameters (e.g., spatial blob thresholds) were tuned on Row 1, frozen, and then applied blindly to the Row 2 held-out set. This workflow is summarized in Figure~\ref{fig:whole_image_protocol_overview}, and the exact sample distributions are detailed in Table~\ref{tab:dataset_statistics}.
