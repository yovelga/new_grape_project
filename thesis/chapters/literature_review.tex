...
\begin{figure}[hbt!]
    ...
    \scalebox{0.82}{%
    \begin{tikzpicture}[
        node distance=1.0cm, % Slightly larger distance for better spacing
        box/.style={
            rectangle, draw=black, rounded corners,
            text width=4.0cm, % width for text, larger than before
    ...
    \node (crack) [box] {\textbf{1. Skin Cracking}\\ Micro-fissures expose sugars};
    \node (infection) [box, right=of crack] {\textbf{2. Infection}\\ Yeast/Bacteria colonization};
    \node (rot) [box, right=of infection] {\textbf{3. Sour Rot}\\ Tissue decay and browning};
    \node (spread) [box, right=of rot] {\textbf{4. Cluster Spread}\\ Adjacent berries infected};
...
\begin{itemize}
    ...
    \item \textbf{SWIR (Short-Wave Infrared)} - wavelengths above 1000~nm, responsive to moisture absorption and biochemical composition.
\end{itemize}
...
\medskip
\noindent
By combining the rich spectral data of HSI with advanced machine learning techniques, it is possible to transition from late-stage, post-symptomatic visual inspection to early-stage detection at the time of measurement. This establishes the necessary technological foundation for the field-ready crack detection framework proposed in this thesis.
...
\medskip
\noindent
The literature review reveals a significant conceptual and methodological disconnect. On one hand, the physiological and biomechanical mechanisms driving grape cracking are well documented, establishing that internal pressure imbalances and micro-structural failures precede macroscopic skin rupture. On the other hand, while hyperspectral imaging has proven highly capable of detecting latent internal defects, its application has been largely restricted to reactive, post-symptomatic identification or highly controlled laboratory settings. Consequently, a critical research gap persists: \textbf{there is currently no established, non-destructive framework capable of providing proactive, early-warning detection of grape cracking susceptibility under realistic preharvest field conditions.} 

\medskip
\noindent
The present study is directly positioned to bridge this gap. By integrating VIS--NIR hyperspectral imaging with optimized classical machine learning and spectral wavelength selection, this research moves beyond reactive defect detection. Unlike prior works, the proposed framework emphasizes early-stage identification under actual vineyard variability. As conceptualized in Figure~\ref{fig:pipeline_overview}, this targeted methodology offers a distinct advantage: rather than relying on data-hungry deep learning models that risk severe overfitting on limited field data, it leverages the data-efficiency and interpretability of classical algorithms. By explicitly linking highly sensitive pixel-level spectral classification with robust whole-image spatial aggregation, this approach filters out background clutter and mitigates domain shift. Ultimately, this thesis introduces a novel, computationally efficient, and field-ready early warning tool designed to identify the physiological onset of grape cracking before it translates into irreversible economic loss.