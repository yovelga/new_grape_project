...
\medskip
\noindent
\textbf{Terminology (Early vs.\ Late Acquisition).} In this thesis, \textit{detection/identification} refers to classification at the \textit{time of measurement} using hyperspectral signatures (e.g., pixel-level crack identification and whole-image inference under early- vs.\ late-stage conditions). Here, \textit{early} and \textit{late} denote \textit{acquisition-stage subsets relative to crack progression}: for a given grape/cluster identity, \textit{early} corresponds to the first acquisition at which cracking is absent or only minimal/incipient (i.e., the first time a small crack is recorded), whereas \textit{late} corresponds to acquisitions where cracking is advanced and macroscopically visible. These terms describe crack status at the time of measurement (and typically align with earlier- vs.\ later-season acquisition periods), not a future outcome.
