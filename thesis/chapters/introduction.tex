...
\noindent
This thesis addresses two related but distinct analysis settings. \textit{Identification/detection} concerns recognising existing crack tissue based on its hyperspectral signature and can be naturally posed as a pixel-level classification problem when representative labels are available. \textit{Early-warning screening}, in contrast, targets early-stage field assessment under realistic vineyard conditions, where direct pixel-wise segmentation of complex scenes is often unstable for example, background clutter (e.g., leaves, wood, and artificial materials) acquisition-dependent domain shifts (illumination, viewing geometry) which can dominate individual pixel-level outputs and lead to unreliable image-level decisions. For this reason, the early-warning setting is approached through robust single-acquisition representations that aggregate information at the grape-cluster/image level. 
% In particular, early-stage acquisitions are not a focal point for clear field identification because reliable field indicators are often not yet present, whereas late-stage acquisitions typically allow clearer field identification.
Unlike late-stage cracking, which presents easily observable structural damage, early-stage defects lack reliable visual indicators. Therefore, early-warning detection relies on the potential of HSI to capture subtle, pre-visual spectral changes before macroscopic symptoms fully develop.
...
% \medskip
% \noindent
% Throughout this thesis, \textit{detection} refers to the classification of cracked tissue using its hyperspectral signature at the time of measurement. To analyze the progression of the damage, images of the grape clusters are categorized into two main phases. \textit{Early-stage} refers to the first image captured when cracking initially begins, representing the earliest observable signs of the disorder. In contrast, \textit{late-stage} refers to images taken after cracking is already established, representing a more advanced and severe condition.
...
While previous studies have demonstrated the potential of hyperspectral imaging (HSI) for agricultural anomaly detection, most research has been confined to controlled laboratory settings or detached fruit (What was done). A critical knowledge gap remains regarding the operational translation of these methods to complex, unconstrained field conditions. Specifically, existing direct pixel-level approaches struggle to cope with severe environmental clutter (e.g., varying illumination, background materials) and lack the robustness required to identify early-stage, pre-visual cracking in intact grape clusters (What is missing). To bridge this gap, this thesis introduces a novel spatial-aggregation framework combined with dimensionality reduction, shifting the detection paradigm from idealized laboratory analysis to practical, field-ready monitoring (Why this work is needed). 
...
\begin{itemize}[leftmargin=*]
    ...
    \item A standardized data preprocessing pipeline was established to clean the raw camera measurements and ensure consistency. This included filtering out noisy sensor bands by restricting the data to the most reliable wavelength range (450--925\,nm), and applying a normalization technique (SNV) to neutralize the effects of varying field illumination and shadows, enabling accurate comparisons across all experiments.
    % \item A field-acquired hyperspectral dataset was collected and curated at both pixel and whole-image levels, including manual pixel labeling and full-scene vineyard acquisitions, and was used to benchmark supervised classifiers under leave-one-group-out protocols and row-separated calibration/test evaluation, demonstrating reliable crack discrimination at the spectral-signature level (Chapter~\ref{ch:results}, Section~\ref{sec:pixel-level-classification}).
    \item A comprehensive hyperspectral dataset was collected directly from the vineyard, comprising both manually labeled isolated pixels and full-scene images. 
    \item Supervised classifiers were trained and rigorously evaluated on this dataset. By testing the models on distinct, unseen vineyard rows, the study successfully demonstrated reliable crack discrimination based on spectral signatures (Chapter~\ref{ch:results}, Section~\ref{sec:pixel-level-classification}).
