...
for the \textit{Cracked} class (positive class). Under class imbalance, PR--AUC\textsubscript{CRACK} is treated as the primary separability metric (see Section~\ref{subsubsec:metrics_rationale}).
...
The next experiment investigates the effect of spectral dimensionality reduction on this same end-to-end pipeline by applying Backward Feature Selection (BFS) and tracking CRACK metrics as a function of the number of retained wavelengths.
...
At each BFS step, the pipeline performance was recorded using CRACK-class metrics, with \textbf{CRACK PR--AUC} treated as the primary objective (Section~\ref{subsec:dimensionality_reduction}).
...
corresponding to the most aggressive dimensionality reduction evaluated
