...
\noindent
This chapter reports quantitative results following the experimental pipeline described in Section~\ref{sec:modeling_evaluation}: spectral characterization, pixel-level classification, whole-image detection, and wavelength-selection analysis.

\noindent
\textbf{Presentation note:} The main text presents mean values truncated to two decimal places for clarity. Full cross-validation statistics (mean $\pm$ standard deviation, fold-level data) are provided in Appendix~\ref{app:cv_statistics}.

\subsection{Baselines}
\label{subsec:results_baselines}

\noindent
Each set of results is reported relative to the explicit baseline defined below:

\begin{itemize}

\item \textbf{Pixel-level classification baseline.}
The baseline model is \textbf{XGBoost} trained on the \textbf{full SNV-normalized hyperspectral signature} (450--925\,nm, 159 bands). Binary-task performance is reported in the XGBoost rows of Table~\ref{tab:pixel_metrics_unbalanced}. Results for extended labeling and balancing regimes appear in Table~\ref{tab:pixel_metrics_unbalanced_all}, with balanced-training baselines provided in Appendix~\ref{app:balanced_results}.

\item \textbf{Whole-image crack-detection baseline.}
Whole-image detection was evaluated using the aggregation pipeline described in Section~\ref{subsec:full_image_pipeline}, applied to pixel-level probability maps from the full-spectrum model without wavelength selection. Early/late calibration and held-out test performance are summarized in Table~\ref{tab:full_image_early_late_metrics}, with dataset composition reported in Table~\ref{tab:full_image_dataset_composition}.

\item \textbf{BFS wavelength-selection reference.}
BFS performance is reported as a function of the \textbf{number of retained wavelengths} relative to the \textbf{full-spectrum baseline}. Elimination trajectories are shown in Figures~\ref{fig:bfs_dual_metric} and~\ref{fig:bfs_threshold_markers}, with derived wavelength-count thresholds summarized in Table~\ref{tab:bfs_thresholds_summary}. Whole-image test-set results at representative wavelength counts, including the 159-band baseline, are reported in Table~\ref{tab:full_image_wavelength_results}.
...
\noindent
Supervised pixel-level classification addresses \hyperref[hyp:pixel_level_feasibility]{Hypothesis~2}, following the LOGO cross-validation protocol in Section~\ref{sec:modeling_evaluation}. PR--AUC\textsubscript{CRACK} is the primary metric.
...
\noindent
Table~\ref{tab:pixel_metrics_unbalanced} reports performance across LOGO folds (unbalanced class distribution). Balanced-training results are in Appendix~\ref{app:balanced_results}.
...
\noindent
Using the protocol described in Section~\ref{subsec:full_image_pipeline}, whole-image detection was assessed under early- and late-stage conditions, addressing \hyperref[hyp:whole_image_robustness]{Hypothesis~3}. The primary metric is \textbf{F1\textsubscript{CRACK}}. Dataset composition is in Table~\ref{tab:full_image_dataset_composition}; supervised and autoencoder results are in Tables~\ref{tab:full_image_early_late_metrics} and~\ref{tab:full_image_autoencoder_metrics}, respectively.
