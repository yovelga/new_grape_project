...
\noindent
To assess generalization to unseen biological samples, Leave-One-Group-Out (LOGO) cross-validation was applied at the grape-cluster acquisition level. LOGO was selected due to the limited number of available hyperspectral images, which precluded a simple held-out test set while still requiring rigorous evaluation of cluster-level generalization. All pixels extracted from the same hyperspectral image (corresponding to a unique grape cluster) were assigned exclusively to either the training set or the test set within each fold. This grouping strategy ensured that evaluation was performed on spectral signatures from previously unseen clusters, preventing spatial or spectral leakage between partitions.

\medskip
\noindent
For the multi-class formulation with background classes, a domain-aware variant was used: LOGO folds were defined over grape-containing acquisitions, while non-grape background samples were assigned to a fixed grouped holdout split (80/20) to prevent leakage across semantically distinct domains.

\subsubsection{Evaluation Metrics}

\noindent
Classification performance was quantified using metrics selected for their interpretability under class imbalance. The primary ranking metric for pixel-level classification was \textbf{Precision--Recall Area Under the Curve (PR--AUC)} for the \textit{Cracked} class, computed with \textit{Cracked} treated as the positive class. PR--AUC was preferred over ROC--AUC because its baseline corresponds to the positive-class prevalence, making it more informative when the minority class is of primary interest.

\medskip
\noindent
Additional metrics reported included: Accuracy, Balanced Accuracy (macro-average of per-class recall), Precision and Recall for the \textit{Cracked} class, macro-average F1-score, and ROC--AUC for the \textit{Cracked} class. These metrics were computed per fold and summarized as mean $\pm$ standard deviation across folds. Threshold-dependent metrics (Precision, Recall, F1) used the model's default decision rule; for probabilistic classifiers, the standard threshold of 0.5 was applied to predicted probabilities.
...
\begin{itemize}[leftmargin=1.5em,nosep]
    \item \textbf{Binary one-class:} Leave-One-Group-Out (LOGO) at the grape-cluster acquisition level, matching the supervised classification protocol (\S\ref{subsec:pixel_level_framework}).
...
\noindent
Pixels were preprocessed as described in \S\ref{subsec:global_preprocessing}. Specifically, SNV normalization was applied per spectrum, and the retained wavelength range was 450--925\,nm (159 bands at $\sim$3\,nm spacing).
...
\noindent
Dimensionality reduction was investigated as a secondary analysis step at the pixel level, following validation of baseline models on the full spectral representation. As formulated in \S\ref{subsec:pixel_level_framework}, wavelength selection was evaluated separately for binary (\textit{Cracked} vs.\ \textit{Healthy}) and multi-class (with background classes) settings.

\subsubsection{Spectral Preprocessing}

\noindent
Pixels were preprocessed as described in \S\ref{subsec:global_preprocessing}. SNV normalization was applied per spectrum, and the spectral range was restricted to 450--925 nm, yielding 159 bands at approximately 3 nm spacing.
...
\medskip
\noindent
All pixel-level models---including the base classifiers, autoencoder variants, and wavelength-reduced models from dimensionality reduction---were fixed at their trained states. No retraining, tuning, or optimization of these models was performed at the whole-image level.
...
\noindent
To assess generalization across biologically independent samples and prevent spatial
or spectral data leakage, group-aware evaluation protocols were employed.
Where cross-validation was used, Leave-One-Group-Out (LOGO) was applied at the
grape-cluster acquisition level so that all pixels originating from the same
grape cluster (or the same hyperspectral acquisition of a cluster) were assigned
exclusively to either training or test within a fold. For multi-class settings with
explicit background/distractor classes, a domain-aware variant was additionally used
in which grape (or CRACK-containing) groups define LOGO folds while non-grape samples
are assigned to a fixed grouped hold-out split.

\medskip
\noindent
This strategy ensures that evaluation is performed on previously unseen biological
samples and provides a realistic estimate of model performance under field
