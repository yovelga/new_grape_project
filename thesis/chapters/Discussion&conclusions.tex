...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/results/discussion/multi_vs_binary_cluster_42.JPG}
    \caption{Multi-class vs.\ binary crack-probability maps on a \textbf{healthy (non-cracked)} cluster. Left: Canon R5 RGB reference. Centre: multi-class XGBoost overlay. Right: binary XGBoost overlay. The binary model produces widespread false positives across background elements (soil, branches, lighting gradients), whereas the multi-class model confines detections to a small number of localised regions. Yellow pixels indicate elevated CRACK probability.}
    \label{fig:multi_vs_binary_nocrack}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/results/discussion/multi_vs_binary_cluster_19.JPG}
    \caption{Multi-class vs.\ binary crack-probability maps on a \textbf{cracked} cluster. Left: Canon R5 RGB reference. Centre: multi-class XGBoost overlay. Right: binary XGBoost overlay. Both models detect the defect region, but the binary model additionally flags large background patches, severely reducing precision. The multi-class formulation isolates the true defect blob while suppressing background noise.}
    \label{fig:multi_vs_binary_crack}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/results/discussion/Patch_map_output.JPG}
    \caption{Example output of the spatial aggregation pipeline. The image illustrates the patch-based density map overlaid on a field scene, where the highlighted patch regions satisfy the crack-density criterion and trigger a positive detection. This visualization demonstrates how the pipeline transitions from a noisy, pixel-level probability map to a structured, spatially coherent image-level decision.}
    \label{fig:patch_map_output}
\end{figure}
...
To quantify this, the pairwise Jaccard similarity was recomputed after binning the selected wavelengths into 5, 10, and 20\,nm windows (Table~\ref{tab:binned_jaccard_summary}; Figure~\ref{fig:binned_jaccard_summary}; full matrices in Appendix Tables~\ref{tab:binned_jaccard_30_bin5}--\ref{tab:binned_jaccard_11_bin20}).
...
\begin{table}[!htbp]
\centering
\caption{Mean pairwise Jaccard similarity across 5 BFS seeds for exact and binned wavelength matching.}
\label{tab:binned_jaccard_summary}
\begin{tabular}{llrr}
\toprule
Top-K & Bin & Mean Jaccard & Std \\
\midrule
30 & Exact & 0.493 & 0.102 \\
30 & 5\,nm  & 0.562 & 0.111 \\
30 & 10\,nm & 0.697 & 0.081 \\
30 & 20\,nm & 0.811 & 0.067 \\
\midrule
11 & Exact & 0.398 & 0.062 \\
11 & 5\,nm  & 0.536 & 0.082 \\
11 & 10\,nm & 0.639 & 0.071 \\
11 & 20\,nm & 0.640 & 0.065 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/results/feature_selection/binned_jaccard_summary.png}
\caption{Effect of wavelength binning on cross-seed Jaccard similarity for the top-30 and top-11 BFS subsets. Error bars denote the standard deviation of all 10 pairwise comparisons.}
\label{fig:binned_jaccard_summary}
\end{figure}
...
The proposed spectral-spatial cascade is illustrated in Figure~\ref{fig:sam2_cnn_pipeline}.
