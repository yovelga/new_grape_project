...
\noindent
The initial finding of this study---that cracked tissues exhibit significantly lower reflectance across both the VIS and NIR regions compared to healthy tissues---is strongly rooted in the biomechanical and physiological consequences of fruit cracking.
In the VIS region (400--700\,nm), the depressed reflectance of cracked tissue is primarily driven by the rupture of the waxy cuticle and the subsequent oxidation of sub-epidermal cells.
Once this protective barrier is breached, exposed phenolic compounds undergo rapid oxidative browning \cite{santos2023molecular}, which substantially increases light absorption.
Furthermore, the loss of the smooth cuticle layer eliminates specular reflectance, causing the cracked area to appear darker.
In the NIR region (700--850\,nm), where the spectral separability is most pronounced, the governing factors are cellular structure and water content \cite{thenkabail2016hyperspectral}.
Healthy mesocarp tissue acts as a strong diffuse reflector due to internal scattering at air--cell interfaces and high turgor pressure.
Cracking induces a localized collapse of this cellular architecture; the rapid loss of turgor and leakage of fluids destroy the scattering interfaces, resulting in a dramatic drop in NIR reflectance \cite{Chang2019,YU20251506}.
...
In whole-image field scenes, cracked pixels typically constitute less than $0.001\%$ of the dataset.
...
\subsection{Classifier Selection and Spatial Validation}
\noindent
Within the supervised paradigm, the superiority of tree-based ensembles (XGBoost, Random Forest) over linear models (PLS-DA, Logistic Regression) underscored the highly non-linear and collinear nature of the multi-class spectral boundaries.
However, the ultimate selection of XGBoost as the primary classifier was driven as much by operational scalability as by accuracy.
During preliminary trials, Support Vector Machines (SVM) achieved competitive pixel-level accuracy.
Yet, their inference complexity ($O(n^2)$ to $O(n^3)$) proved prohibitive for whole-image deployment.
A single field scene contains over 260,000 pixels ($512 \times 512$)---exceeding the total size of the extracted pixel-level training dataset.
SVM inference on a single full image was computationally unfeasible for high-throughput screening.
Tree-based architectures, alongside shallow Multi-Layer Perceptrons (MLPs), provided the necessary inference speed to make full-image field deployment practical, highlighting the critical transition from laboratory-scale accuracy to operational field constraints.

\medskip\noindent
Crucially, this pixel-level robustness was validated using a strict Leave-One-Group-Out (LOGO) cross-validation strategy, where each group corresponded to a distinct physical grape cluster.
In agricultural computer vision, standard random K-fold splits frequently suffer from spatial data leakage, as adjacent pixels from the same physical object fall into both training and validation sets, artificially inflating performance.
By enforcing a cluster-level separation during cross-validation, the reported pixel-level metrics represent true generalization to unseen biological samples rather than mere spatial memorization.
...
\medskip\noindent
These custom visual tools were not merely conveniences; they were absolute methodological necessities.
Visual verification was the only way to guarantee that the extracted hyperspectral segments aligned perfectly with the physical berries, and that the morphological filters were behaving as mathematically intended before feature extraction.
This rigorous, manual visual-validation loop was the invisible foundation of the study, ensuring that the downstream classifiers learned from accurate, biologically meaningful data rather than processing artifacts.
...
\medskip\noindent
Finally, a highly promising direction involves deep-learning-based spatial filtering to refine the whole-image detection pipeline.
Preliminary investigations during this study explored a multi-stage cascade: utilizing the spectral model to detect suspected defect blobs, extracting their centroids, applying the Segment Anything Model 2 (SAM2) to isolate the local structure, and feeding the corresponding RGB bounding box into a Convolutional Neural Network (CNN) to classify the object as 'grape' or 'non-grape' (e.g., dry wood).
While the CNN achieved excellent accuracy on isolated calibration segments---testing various inputs such as tight bounding boxes, 5-pixel padded boxes, and fully masked background segments---it suffered a severe performance drop when deployed on whole-image field scenes.
This discrepancy highlights a critical domain shift: spatial models trained on cleanly extracted ROIs struggle with the overlapping geometries, shadows, and contextual noise of a full canopy.
Nevertheless, the core concept of a spectral-spatial cascade---using the hyperspectral pixel model as a highly sensitive primary detector, and an RGB-based CNN as a geometric secondary filter---remains conceptually sound.
Future iterations of this architecture, potentially utilizing end-to-end training, could effectively eliminate the remaining false positives and yield a near-perfect early-warning detection system.