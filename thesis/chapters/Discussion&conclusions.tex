...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/results/discussion/multi_vs_binary_cluster_42.JPG}\\[0.5em]
    \includegraphics[width=0.75\textwidth]{figures/results/discussion/multi_vs_binary_cluster_19.JPG}
    \caption{Multi-class vs.\ binary crack-probability maps. Top: a \textbf{healthy (non-cracked)} cluster; bottom: a \textbf{cracked} cluster. In each row---Left: Canon R5 RGB reference; Centre: multi-class XGBoost overlay; Right: binary XGBoost overlay. The binary model produces widespread false positives across background elements, whereas the multi-class formulation confines detections to localised regions and isolates the true defect blob. Yellow pixels indicate elevated CRACK probability.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/materials_methods/pipeline/TP_example_3.png}
