...
\begin{itemize}
  \item \textbf{Sections~\ref{sec:grape_cracking}--\ref{sec:physiomechanics} (The Biological \& Economic Context):} Explore grape cracking as a physiological disorder, detailing its biomechanical mechanisms and the substantial economic impact that necessitates proactive monitoring.
  \item \textbf{Sections~\ref{sec:precision_agriculture} \& \ref{sec:hyperspectral_imaging} (Sensing Technologies):} Introduce precision agriculture concepts and highlight Hyperspectral Imaging (HSI) as a powerful non-destructive sensing modality to address this agricultural need.
  \item \textbf{Section~\ref{sec:ml_hsi} (Computational Approaches):} Examines relevant machine learning paradigms, as extracting actionable insights from HSI requires advanced analytics.
  \item \textbf{Sections~\ref{sec:related_work} \& \ref{sec:research_gap} (Related Work \& Research Gap):} Review existing literature on fruit defect detection, culminating in the identification of the critical research gap addressed by this thesis: the absence of robust, field-ready frameworks for early-stage crack detection.
\end{itemize}
...
\cite{OWOYEMI2024113013,Hamie2022SugranineteenMaturation}. While these works focus primarily on postharvest quality, they provide
