...
# LAST_WEEK_COL = "10.07.24"
LAST_WEEK_COL = "01.07.24"
OUTPUT_CSV = r"C:\Users\yovel\Desktop\Grape_Project\src\preprocessing\prepare_dataset_for_full_image_classification\hole_image\first_date_dataset\early_detection_dataset.csv"
...
def generate_single_date_dataset(
    excel_path: str,
    sheet_name: Optional[str],
    grape_col: str,
    target_date: str,
    ...
    """Generate a dataset CSV for a single date with all labels set to 0.

    This is useful for training on class 0 (non-cracked) samples only.
    """
    df = read_excel(excel_path, sheet_name)
    ...
    # create grape_id column and normalize
    df["grape_id"] = df[grape_col].astype(str).str.strip()
    # drop rows with missing grape_id
    df = df[df["grape_id"].notna() & (df["grape_id"].str.strip() != "")]
    ...
    # build image_path using target_date
    df["image_path"] = df["grape_id"].apply(lambda gid: build_folder_path(base_raw_dir, gid, target_date))
    ...
    # drop rows with empty image_path and duplicated grape_id
    output_df = output_df[output_df["image_path"].notna() & (output_df["image_path"].str.strip() != "")]
    output_df = output_df.drop_duplicates(subset=["grape_id"])

    # save
    output_df.to_csv(output_csv, index=False)
    print(f"Saved single-date dataset (date={target_date}, label=0) to {output_csv} with {len(output_df)} rows.")
    return output_df


if __name__ == "__main__":
    try:
        # Generate single-date dataset with all label=0 for date 01.07.24
        generate_single_date_dataset(
            EXCEL_PATH,
            SHEET_NAME,
            GRAPE_ID_COL,
            target_date="01.07.24",
