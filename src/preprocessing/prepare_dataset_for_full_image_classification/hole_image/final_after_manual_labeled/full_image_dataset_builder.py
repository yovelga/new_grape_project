"""
Full-image dataset builder for MANUALLY LABELED images.

Goal
----
Generate EXACTLY 5 CSV files for full-image crack detection from manually labeled data.
- Train: Row 1
  - Positive (label=1): Images marked as cracked in the CSV (value=1)
  - Negative (label=0): Images with week_date <= 25.07.24 that are NOT marked as cracked
- Val: Row 1 (two variants - EARLY and LATE)
  - Positive grapes: ONE sample per grape (earliest/latest crack week for that grape, label=1)
...
- Test: Row 2 (two variants)
  - EARLY: for each grape_id choose the FIRST crack week. If never cracked -> random week <= 25.07.24, label=0
  - LATE:  for each grape_id choose the LAST  crack week. If never cracked -> random week <= 25.07.24, label=0

Inputs
------
1) CSV file with week columns (dd.mm.yy) and a "Grape ID" column.
   Value of 1 indicates crack, empty indicates no crack.
...
GLOBAL CONSTRAINT: All label=0 samples must have week_date <= 25.07.24
"""
...
class Config:
    ...
    base_raw_dir: str = str(_PROJECT_ROOT / r"data/raw")

    # Dates
    august_cutoff: str = "25.07.24"   # dd.mm.yy: used for identifying crack consideration period

    # Splits
    val_ratio: float = 1.0  # Set to 1.0 to create ONLY validation sets (no training data)
    ...
    # Output
    out_dir: Path = Path("dataset_csvs")
...
CFG = Config(
    csv_path=SCRIPT_DIR / "crack_labels_from_txt.csv",
...
def is_cracked_cell(v) -> bool:
    """CSV crack indicator: 1 for crack, empty for not-crack."""
    if pd.isna(v):
        return False
    if isinstance(v, str):
        t = v.strip()
...
def get_crack_pairs_from_csv(df: pd.DataFrame, week_cols: List[str], grape_id_col: str) -> Set[Tuple[str, str]]:
    """Extract (grape_id, week_date) pairs where crack is marked in the CSV."""
    crack_pairs = set()

    for _, row in df.iterrows():
        grape_id = str(row[grape_id_col]).strip()
        for wk in week_cols:
            if is_cracked_cell(row[wk]):
                crack_pairs.add((grape_id, wk))
...
def build_row1_train_dataset(
    ...
    # Override with crack positives (only for train grapes)
...
def build_row2_test_early_late(
    ...
    # Filter row2 only
    df = df_csv.copy()
    df["grape_id"] = df[grape_id_col].astype(str).str.strip()
...
def main() -> None:
    ...
    if CFG.grape_id_col not in df_csv.columns:
        raise ValueError(f"Grape ID col '{CFG.grape_id_col}' not found. Available: {list(df_csv.columns)}")

    # Detect and sort week columns
    week_cols = sort_week_cols([c for c in df_csv.columns if is_week_column(c)])
    if not week_cols:
        raise ValueError("No week columns detected (expected dd.mm.yy).")

    # Normalize grape_id
    df_csv["grape_id"] = df_csv[CFG.grape_id_col].astype(str).str.strip()
    df_csv["row"] = df_csv["grape_id"].apply(extract_row_from_grape_id)

    # Row lists
    row1_grapes = sorted(df_csv.loc[df_csv["row"] == CFG.trainval_row, "grape_id"].dropna().unique().tolist())
    row2_grapes = sorted(df_csv.loc[df_csv["row"] == CFG.test_row, "grape_id"].dropna().unique().tolist())
    ...
    print(f"- Row2 grapes detected: {len(row2_grapes)}")

    # Extract crack pairs from CSV
    crack_pairs = get_crack_pairs_from_csv(df_csv, week_cols, CFG.grape_id_col)
    crack_pairs_row1 = {(gid, wk) for (gid, wk) in crack_pairs if extract_row_from_grape_id(gid) == 1}
    crack_pairs_row2 = {(gid, wk) for (gid, wk) in crack_pairs if extract_row_from_grape_id(gid) == 2}
    print(f"- Total crack pairs: {len(crack_pairs)} (Row1: {len(crack_pairs_row1)}, Row2: {len(crack_pairs_row2)})")
    ...
    if len(row1_train) > 0:
        ...
        n_pos_train = n_neg_train = n_unique_train = 0
    ...
    if len(row1_train) > 0:
        ...
        print("GRAPE_ID OVERLAP CHECK")
        print("-" * 80)
        print("✓ No training data (val_ratio = 1.0) - overlap check skipped")
    ...
    if not constraint_violations:
        print("✓ All negative samples satisfy week_date <= 25.07.24")
    ...
    if len(row1_train) > 0:
        ...
        val_early_image_paths = set(val_early["image_path"].unique())
        val_late_image_paths = set(val_late["image_path"].unique())
        print(f"Train unique images: 0")
        print(f"Val Early unique images: {len(val_early_image_paths)}")
        print(f"Val Late unique images: {len(val_late_image_paths)}")
        print("✓ No training data (val_ratio = 1.0) - image overlap check skipped")
