"""
train_xgboost_row1_segments_11_channels_minus_half_percent.py

XGBoost training script for pixel-level classification using ONLY ROW 1 data.
*** -0.5% MODEL: 11 channels selected via BFS (PR-AUC: 0.9878, 99.3% of best) ***

Segment-level samples (mask_path) with configurable SPLIT_LEVEL grouping.

KEY DATA LEAKAGE RULES:
  - Segment overlap between train/val is ALWAYS 0 (no pixel leakage)
  - Cluster overlap depends on SPLIT_LEVEL:
    * "segment" (default): cluster overlap ALLOWED
    * "cluster": cluster overlap MUST be 0

OUTPUT STRUCTURE:
  experiments/xgboost_row1/bfs_11_channels_minus_half_percent/<TIMESTAMP>/<dataset>/<balance_mode>/

Features:
- Filters data to ROW 1 only before preprocessing
- Uses GroupShuffleSplit with SPLIT_LEVEL for train/val split (80/20)
...
- *** USES TOP 11 CHANNELS FROM BFS RESULTS ***

Based on BFS results from 2026-02-01_20-53-40:
Iteration 148: 11 features, CRACK PR-AUC = 0.9877827875045431 (~99.3% of best)
...
# Path: src/models/classification/pixel_level/train_xgboost_row_1_for_full_image_test_20_80/train_xgboost_row1_segments_11_channels_minus_half_percent.py
...
def setup_logging(experiment_dir: Path) -> Tuple[Path, Path]:
    ...
    with open(stdout_log, 'w', encoding='utf-8') as f:
        f.write(f"=" * 80 + "\n")
        f.write(f"XGBoost ROW 1 Training Log - BFS 11 CHANNELS (-0.5%)\n")
        f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"=" * 80 + "\n\n")
    
    with open(stderr_log, 'w', encoding='utf-8') as f:
        f.write(f"=" * 80 + "\n")
        f.write(f"XGBoost ROW 1 Training Error Log - BFS 11 CHANNELS (-0.5%)\n")
        f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"=" * 80 + "\n\n")
...
# Selected channels from BFS results (iteration 148 - 11 features, -0.5% performance)
# CRACK PR-AUC = 0.9877827875045431 (99.3% of best)
# Exact column names computed from BFS log (remaining after removals)
BFS_SELECTED_CHANNELS = [
    "452.25nm", "536.82nm", "545.62nm", "580.90nm", "634.11nm",
    "729.53nm", "732.53nm", "738.53nm", "756.56nm", "892.95nm", "917.42nm"
]

print(f"[BFS] Using {len(BFS_SELECTED_CHANNELS)} channels selected via BFS")
print(f"[BFS] Channels: {BFS_SELECTED_CHANNELS}")


# ==================== STARTUP LOGGING FUNCTIONS ====================

def print_global_run_config() -> None:
    """
    Sanity-check log #1: Print global run configuration.
    Call at the start of main() to verify settings before any processing.
    """
    print("\n" + "="*60)
    ...
    print(f"VAL_SIZE:          {VAL_SIZE:.0%}")
    print(f"RANDOM_STATE:      {RANDOM_STATE}")
    print(f"MAX_SAMPLES_PER_CLASS: {MAX_SAMPLES_PER_CLASS:,}")
    print(f"BFS Channels:      {len(BFS_SELECTED_CHANNELS)} selected")
    print(f"Expected PR-AUC:   0.9877827875045431 (99.3% of best)")
    print("="*60 + "\n")


def print_row_filter_summary(filter_info: Dict) -> None:
    """
    Sanity-check log #2: Print row filter summary.
    Call after load_and_filter_row1() to verify data filtering.
    """
    print("\n" + "-"*50)
    print("ROW FILTER SUMMARY")
    print("-"*50)
    print(f"Original rows:      {filter_info['original_rows']:,}")
    print(f"ROW 1 samples:      {filter_info['row1_samples']:,}")
    print(f"Detection method:   {filter_info['row_detection_method']}")
    if filter_info.get('rows_excluded'):
        print(f"Rows excluded:      {filter_info['rows_excluded']}")
    print("-"*50 + "\n")
...
def print_segment_cluster_summary(
    ...
    """
    Sanity-check log #3: Print segment/cluster extraction summary.
    Call after extracting segment_ids and cluster_ids to verify grouping.
    """
    unique_segments = np.unique(segment_ids)
    unique_clusters = np.unique(cluster_ids)
    
    print("\n" + "-"*50)
    print("SEGMENT / CLUSTER SUMMARY")
    print("-"*50)
    print(f"Total samples:      {len(segment_ids):,}")
    ...
    print(f"Samples per segment: {len(segment_ids) / len(unique_segments):.1f} (avg)")
    print(f"Segments per cluster: {len(unique_segments) / len(unique_clusters):.1f} (avg)")
    print("-"*50 + "\n")
...
def print_pre_split_summary(
    ...
    """
    Sanity-check log #4: Print pre-split summary.
    Call just before create_group_train_val_split() to verify all data is ready.
    """
    print("\n" + "-"*50)
    print("PRE-SPLIT SUMMARY")
    print("-"*50)
...
# experiments/xgboost_row1/bfs_11_channels_minus_half_percent/<TIMESTAMP>/<dataset>/<balance_mode>/
EXPERIMENTS_ROOT = Path(r"C:\Users\yovel\Desktop\Grape_Project\experiments\xgboost_row1")
RUN_NAME = "bfs_11_channels_minus_half_percent"

TIMESTAMP = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
EXPERIMENT_DIR = EXPERIMENTS_ROOT / RUN_NAME / TIMESTAMP

# ---- Split configuration ----
# SPLIT_LEVEL controls how we group samples for train/val split:
#   "segment" (default): Group by segment_id (mask_path). Cluster overlap ALLOWED.
#   "cluster": Group by cluster_id. Cluster overlap MUST be 0.
SPLIT_LEVEL = "segment"  # <-- Change to "cluster" for cluster-level split
...
def select_bfs_channels(features: np.ndarray, feature_names: List[str]) -> Tuple[np.ndarray, List[str]]:
    ...
    print(f"[CHANNEL_SELECT] Selected channels: {selected_names}")
...
# Maximum number of sample IDs to store in manifests (for compact output)
_MAX_SAMPLE_IDS = 50
...
def create_group_train_val_split(
    ...
    """
    Create train/val split with configurable grouping level.
    
    LEAKAGE RULES:
      - Segment overlap is ALWAYS 0 (no pixel leakage)
      - Cluster overlap depends on split_level:
        * "segment": cluster overlap ALLOWED (same cluster can appear in both)
        * "cluster": cluster overlap MUST be 0
    
    Args:
        X: Feature matrix (n_samples, n_features)
        y: Label array (n_samples,)
        segment_ids: Segment ID for each sample (mask_path)
        cluster_ids: Cluster ID for each sample
        split_level: "segment" or "cluster"
    ...
    print(f"[SPLIT] SPLIT_LEVEL: {split_level}")
    print(f"[SPLIT] Val size: {val_size:.0%}, Random state: {random_state}")
    
    # Choose grouping column based on split_level
    if split_level == "cluster":
        groups = cluster_ids
        group_name = "cluster_id"
    elif split_level == "segment":
        groups = segment_ids
        group_name = "segment_id"
    else:
        raise ValueError(f"Invalid SPLIT_LEVEL: {split_level}. Use 'segment' or 'cluster'.")
    
    unique_groups = np.unique(groups)
    print(f"[SPLIT] Grouping by: {group_name} ({len(unique_groups)} unique groups)")
    
    # GroupShuffleSplit by the chosen grouping
    ...
    # Get segment and cluster sets for both splits
    ...
    # Check segment overlap (MUST always be 0)
    segment_overlap = train_segments & val_segments
    n_segment_overlap = len(segment_overlap)
    if n_segment_overlap > 0:
        raise ValueError(f"LEAKAGE: {n_segment_overlap} segments in both train and val!")
    
    # Check cluster overlap (only error if split_level="cluster")
    ...
    if split_level == "cluster" and n_cluster_overlap > 0:
        raise ValueError(f"LEAKAGE: {n_cluster_overlap} clusters in both train and val!")
    
    # Print summary
    print(f"[SPLIT] Train: {len(train_idx):,} samples, {len(train_segments)} segments, {len(train_clusters)} clusters")
    print(f"[SPLIT] Val: {len(val_idx):,} samples, {len(val_segments)} segments, {len(val_clusters)} clusters")
    print(f"[SPLIT] Segment overlap: {n_segment_overlap} (must be 0)")
    if split_level == "cluster":
        print(f"[SPLIT] Cluster overlap: {n_cluster_overlap} (must be 0 for cluster split)")
    else:
        print(f"[SPLIT] Cluster overlap: {n_cluster_overlap} (allowed for segment split)")
    
    # Build compact split_info (store samples, not full lists)
    train_seg_list = sorted(train_segments)
    val_seg_list = sorted(val_segments)
    train_clust_list = sorted(train_clusters)
    val_clust_list = sorted(val_clusters)
    ...
    split_info = {
        ...
        # Overlap checks
        "n_segment_overlap": n_segment_overlap,
        "n_cluster_overlap": n_cluster_overlap,
        # Sample IDs (first N only, for debugging)
        "train_segments_sample": train_seg_list[:_MAX_SAMPLE_IDS],
        "val_segments_sample": val_seg_list[:_MAX_SAMPLE_IDS],
        "train_clusters_sample": train_clust_list[:_MAX_SAMPLE_IDS],
        "val_clusters_sample": val_clust_list[:_MAX_SAMPLE_IDS],
    }
...
def train_xgboost(
    ...
    train_info = {
        ...
        "bfs_info": {
            ...
            "bfs_performance": "PR-AUC: 0.9877827875045431 (99.3% of best)",
        },
    }
...
def evaluate_model(
    ...
    metrics = {
        ...
        "bfs_info": {
            "n_selected_channels": len(BFS_SELECTED_CHANNELS),
            "bfs_expected_performance": "PR-AUC: 0.9877827875045431 (99.3% of best)",
        },
    }
...
def write_readme(out_dir: Path, config: DatasetConfig, balance_mode: str, split_info: Dict) -> None:
    ...
    lines = [
        "="*60,
        "XGBoost ROW 1 Training - BFS 11 Channels (-0.5%)",
        "="*60,
        "",
        ...
        f"Expected PR-AUC: 0.9877827875045431 (99.3% of best)",
        "",
        "--- Files ---",
...
def save_artifacts(
    ...
    manifest = {
        ...
        "model_type": "BFS_11_channels_minus_half_percent",
        "split_level": SPLIT_LEVEL,
        ...
        "preprocessing": {
            ...
            "bfs_channel_selection": {
                ...
                "selected_channels": BFS_SELECTED_CHANNELS,
                "bfs_iteration": 148,
                "expected_performance": "PR-AUC: 0.9877827875045431 (99.3% of best)",
            },
        },
    ...
    bfs_info = {
        ...
        "iteration": 148,
        "n_features": 11,
        "expected_crack_pr_auc": 0.9877827875045431,
        "performance_rank": "-0.5% (99.3% of best)",
...
def _run_training():
    ...
    print("=" * 80)
    print("XGBoost ROW 1 Training Script - BFS 11 CHANNELS (-0.5%)")
    print("=" * 80)
    ...
    print(f"Model: BFS 11 channels (-0.5%) - Expected PR-AUC: 0.9878")
