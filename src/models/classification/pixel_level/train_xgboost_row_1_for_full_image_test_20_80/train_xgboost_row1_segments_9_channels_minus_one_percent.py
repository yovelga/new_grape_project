"""
train_xgboost_row1_segments_9_channels_minus_one_percent.py

XGBoost training script for pixel-level classification using ONLY ROW 1 data.
*** -1% MODEL: 9 channels selected via BFS (PR-AUC: 0.9790, 98.4% of best) ***
...
- *** USES TOP 9 CHANNELS FROM BFS RESULTS ***

Based on BFS results from 2026-02-01_20-53-40:
Iteration 150: 9 features, CRACK PR-AUC = 0.9790396675228346 (~98.4% of best)

NO LOGO, NO CV - just one train/val split for final model training.

Output Structure:
  experiments/xgboost_row1/bfs_9_channels_minus_one_percent/<TIMESTAMP>/<dataset>/<balance_mode>/
"""
...
# Path: src/models/classification/pixel_level/train_xgboost_row_1_for_full_image_test_20_80/train_xgboost_row1_segments_9_channels_minus_one_percent.py
...
def setup_logging(experiment_dir: Path) -> Tuple[Path, Path]:
    ...
    with open(stdout_log, 'w', encoding='utf-8') as f:
        f.write(f"=" * 80 + "\n")
        f.write(f"XGBoost ROW 1 Training Log - BFS 9 CHANNELS (-1%)\n")
        f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"=" * 80 + "\n\n")
    
    with open(stderr_log, 'w', encoding='utf-8') as f:
        f.write(f"=" * 80 + "\n")
        f.write(f"XGBoost ROW 1 Training Error Log - BFS 9 CHANNELS (-1%)\n")
        f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"=" * 80 + "\n\n")
...
# Selected channels from BFS results (iteration 150 - 9 features, -1% performance)
# CRACK PR-AUC = 0.9790396675228346 (98.4% of best)
# Exact column names computed from BFS log (remaining after removals)
BFS_SELECTED_CHANNELS = [
    "452.25nm", "536.82nm", "545.62nm", "634.11nm",
    "729.53nm", "732.53nm", "756.56nm", "892.95nm", "917.42nm"
]
...
def print_global_run_config():
    ...
    print(f"BFS preview:    {BFS_SELECTED_CHANNELS}")
    print("=" * 50)
    print()
...
EXPERIMENTS_ROOT = Path(str(_PROJECT_ROOT / r"experiments"))
RUN_NAME = "xgboost_row1/bfs_9_channels_minus_one_percent"
...
def train_xgboost(
    ...
    train_info = {
        ...
        "bfs_info": {
            ...
            "bfs_performance": "PR-AUC: 0.9790396675228346 (98.4% of best)",
        },
    }
...
def evaluate_model(
    ...
    metrics = {
        ...
        "bfs_info": {
            "n_selected_channels": len(BFS_SELECTED_CHANNELS),
            "bfs_expected_performance": "PR-AUC: 0.9790396675228346 (98.4% of best)",
        },
    }
...
def write_readme(out_dir: Path, config: DatasetConfig, balance_mode: str, split_info: Dict, metrics: Dict = None) -> None:
    ...
    lines = [
        "="*60,
        "XGBoost ROW 1 Training - BFS 9 Channels (-1%)",
        "="*60,
        "",
        ...
        f"Expected PR-AUC: 0.9790396675228346 (98.4% of best)",
        "",
        "--- Files ---",
...
def save_artifacts(
    ...
    manifest = {
        ...
        "model_type": "BFS_9_channels_minus_one_percent",
        "split_level": SPLIT_LEVEL,
        ...
        "preprocessing": {
            ...
            "bfs_channel_selection": {
                ...
                "selected_channels": BFS_SELECTED_CHANNELS,
                "bfs_iteration": 150,
                "expected_performance": "PR-AUC: 0.9790396675228346 (98.4% of best)",
            },
        },
    ...
    bfs_info = {
        ...
        "iteration": 150,
        "n_features": 9,
        "expected_crack_pr_auc": 0.9790396675228346,
        "performance_rank": "-1% (98.4% of best)",
...
def _run_training():
    ...
    print("=" * 80)
    print("XGBoost ROW 1 Training Script - BFS 9 CHANNELS (-1%)")
    print("=" * 80)
    ...
    print(f"Model: BFS 9 channels (-1%) - Expected PR-AUC: 0.9790")
