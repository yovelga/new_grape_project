...
for _ in tqdm(range(500), desc="Training Epochs"):
    ae.fit(X_cr_train, X_cr_train)
    train_losses.append(ae.loss_)

joblib.dump(ae, "autoencoder_trained_model_crack.joblib")
print("Autoencoder saved as autoencoder_trained_model_crack.joblib")
...
reconstructed_cr = ae.predict(X_cr_test)
reconstructed_reg = ae.predict(X_reg)
