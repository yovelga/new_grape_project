...
unified_experiment_pipeline.py
...
class ExperimentConfig:
    ...
    OPTIMIZATION STRATEGY: Maximize CRACK PR-AUC (Average Precision)

    

    Since the dataset is highly imbalanced (CRACK is minority class), this pipeline

    optimizes for Precision-Recall performance rather than Accuracy or ROC-AUC:
    ...
    3. eval_metric='aucpr' for XGBoost

    4. Focus on CRACK Precision, Recall, F1, and PR-AUC in reporting
...
EXPERIMENT_DIR = Path(str(_PROJECT_ROOT / f"experiments/unified_experiment_ACC_{TIMESTAMP}"))
...
def get_all_models(config: ExperimentConfig, n_classes: int = 2, crack_class_idx: int = 1) -> List[Tuple[str, Any, bool]]:
    ...
    Get all models for evaluation, optimized for CRACK PR-AUC.
    ...
    if config.models_to_train.get("XGBoost", True):
        ...
        models.append((
            ...
            XGBClassifier(
                ...
                eval_metric="aucpr",  # Changed from mlogloss to PR-AUC for CRACK focus
...
def evaluate_single_fold(
    ...
    Evaluate a single fold with sample weighting for CRACK PR-AUC optimization.
...
def run_experiment(config: ExperimentConfig):
    ...
    for balance_mode in config.balance_modes:
        ...
        for label_setup in config.label_setups:
            ...
            # Prepare labels for this setup (remap for crack_vs_rest)

            y_setup, class_names_setup, class_mapping_setup, crack_class_idx = prepare_data_for_label_setup(
            ...
            print(f"[INFO] Label setup: {label_setup.value}")
...
if __name__ == "__main__":
    ...
    config = ExperimentConfig(
        ...
        label_setups=[
            ...
            # LabelSetup.CRACK_REGULAR_REST,  # TEMP: skip for crack_vs_rest rerun

            LabelSetup.CRACK_VS_REST,

            # LabelSetup.MULTI_CLASS,  # TEMP: skip for crack_vs_rest rerun
