"""
Binary Classification Inference UI

Main entrypoint for the inference application with two tabs:
...
def log_error(msg: str, exc: Optional[Exception] = None):
    """Log error to file with traceback, show friendly message to user."""
    if exc:
...
class InferenceWorker(QThread):
    ...
    finished = pyqtSignal(object)  # prob_map or None
...
class OptunaWorker(QThread):
    """Worker thread for Optuna tuning."""
