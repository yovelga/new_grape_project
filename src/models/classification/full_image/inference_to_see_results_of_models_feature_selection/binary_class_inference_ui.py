...
class InferenceWorker(QThread):
    ...
    def __init__(self, cube, model, preprocess_cfg, target_class_index, required_feature_names=None):
        super().__init__()
        self.cube = cube
        self.model = model
        self.preprocess_cfg = preprocess_cfg
        self.target_class_index = target_class_index
        self.required_feature_names = required_feature_names  # For reduced models
        self._stop = False
...
class VisualDebugTab(QWidget):
    ...
    def _browse_model(self):
        ...
        if file_path:
            ...
            folder_path = QFileDialog.getExistingDirectory(
                self,
                "Select Model Folder (Autoencoder or Reduced Model Package)",
                default_dir
            )
            if folder_path:
    ...
    def _load_selected_model(self):
        ...
        try:
            ...
            # Update status with type-specific styling based on model category
            if model_info.model_type == "Autoencoder":
            ...
            elif model_info.is_reduced:
                # Reduced-feature model: show feature count
                feature_count = len(model_info.required_feature_names) if model_info.required_feature_names else 0
                self.model_status_label.setText(
                    f"✓ REDUCED: {model_info.name} [{feature_count} features]"
                )
                self.model_status_label.setStyleSheet(
                    "color: #e67e22; font-size: 9px; font-weight: bold; "
                    "padding: 3px; background-color: #fef5e7; border-radius: 3px;"
                ...
                # Full-feature model
                self.model_status_label.setText(f"✓ FULL: {model_path_obj.name}")
                self.model_status_label.setStyleSheet(
            ...
            # Show model category in progress label
            category_label = model_info.model_category.upper()
            self.progress_label.setText(f"Model loaded: {model_info.n_classes} classes ({category_label})")
    ...
    def _run_inference(self):
        ...
        try:
            ...
            # Get required feature names for reduced models
            required_feature_names = self.model_manager.get_required_feature_names()
            if required_feature_names:
                self.progress_label.setText(f"Reduced model: {len(required_feature_names)} features")
            
            self.inference_worker = InferenceWorker(
                self.cube, self.model, preprocess_cfg, target_idx,
