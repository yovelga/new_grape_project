...
Supports both full-feature and reduced-feature models with automatic feature alignment.
"""
...
from ..models.feature_alignment import (
...
def _preprocess_pixels(
    ...
    Returns:
        Tuple of:
            ...
            - Filtered wavelengths (C_out,) if wavelength filtering applied, None otherwise
    """
    ...
    if preprocess_cfg.needs_band_selection:
        ...
        # If band_indices provided, we can't determine wavelengths
    elif preprocess_cfg.needs_wavelength_filter and preprocess_cfg.wavelengths is not None:
        pixels, filtered_wavelengths = select_wavelength_range(
            pixels,
    ...
    return pixels.astype(np.float32), filtered_wavelengths
...
def build_prob_map(
    ...
    Supports both full-feature and reduced-feature models:
    - Full-feature models: Use all preprocessed features (required_feature_names=None)
    - Reduced-feature models: Align features to required subset in training order
    ...
    Args:
        ...
        required_feature_names: List of required feature names for reduced models.
            If None, all features are used (full-feature model).
    ...
    Raises:
        ValueError: If cube shape is invalid, target_class_index is out of range,
            or feature alignment fails (missing required features)
    ...
    pixels_preprocessed, filtered_wavelengths = _preprocess_pixels(pixels, preprocess_cfg)
    ...
    # ===== Feature Alignment for Reduced Models =====
    if required_feature_names is not None:
        # Generate feature names from filtered wavelengths
        if filtered_wavelengths is not None:
            full_feature_names = wavelengths_to_feature_names(filtered_wavelengths)
        elif preprocess_cfg.wavelengths is not None:
            # Use original wavelengths if no filtering was applied
            full_feature_names = wavelengths_to_feature_names(preprocess_cfg.wavelengths)
        else:
            raise ValueError(
                "Cannot align features: wavelengths not available in preprocessing config. "
                "For reduced models, wavelengths must be provided."
            )
        
        # Log feature alignment summary
        log_feature_alignment_summary(
            full_feature_count=len(full_feature_names),
            required_feature_names=required_feature_names,
            model_name="build_prob_map"
        )
        
        # Align features to required subset in training order
        align_start = time.time()
        pixels_aligned = align_features_for_model(
            pixels_preprocessed, full_feature_names, required_feature_names
        )
        logger.info(
            f"Feature alignment took {time.time() - align_start:.3f}s, "
            f"{C_preprocessed} features -> {pixels_aligned.shape[1]} features"
        )
        
        # Use aligned features for inference
        pixels_for_inference = pixels_aligned
    else:
        # Full-feature model: use all preprocessed features
        logger.info(f"Full-feature model: using all {C_preprocessed} features")
        pixels_for_inference = pixels_preprocessed
    ...
    for chunk_idx in range(num_chunks):
        ...
        chunk_pixels = pixels_for_inference[start_idx:end_idx]
