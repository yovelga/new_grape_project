...
def load_model_and_scaler(lda_path: str, scaler_path: Optional[str] = None) -> Tuple[object, Optional[object], int, np.ndarray, Optional[float]]:
    ...
    logger.info("Model loaded. classes=%s | pos_idx=%d (CRACK/decay class) | scaler=%s | optimal_threshold=%s",
...
def run_patch_analysis(
    ...
    lda, scaler, default_pos_idx, classes, _ = load_model_and_scaler(lda_path, scaler_path)
...
class HSILDAViewer(QMainWindow):
    ...
    def _on_model_changed(self, index: int):
        ...
        if index >= 0 and index < len(self.available_model_paths):
            ...
            try:
                _, _, pos_idx, classes, _ = load_model_and_scaler(self.lda_path, self.scaler_path)
    ...
    def _run_detection_only(self):
        ...
        try:
            ...
            lda, scaler, default_pos_idx, classes, _ = load_model_and_scaler(self.lda_path, self.scaler_path)
    ...
    def _dataset_start(self):
        ...
        try:
            if not self.lda_path:
                ...
                return
            # Note: Returns (lda, scaler, pos_idx, classes, optimal_threshold) - stored but currently unused
    ...
    def _dataset_display_index(self, idx: int):
        ...
        try:
            ...
            lda, scaler, pos_idx, classes, _ = load_model_and_scaler(self.lda_path, self.scaler_path)
            prob_map = per_pixel_probs(cube, lda, scaler, pos_idx)
