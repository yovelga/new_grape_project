...
# Import SNV preprocessing for binary models
project_path = Path(__file__).resolve().parents[4]
sys.path.insert(0, str(project_path))
from src.preprocessing.spectral_preprocessing import _snv

# ===== Binary model preprocessing config =====
BINARY_WL_MIN = 450  # Minimum wavelength (nm) for binary models
BINARY_WL_MAX = 925  # Maximum wavelength (nm) for binary models
...
def get_wavelengths_from_hdr(hdr_path: str) -> Optional[np.ndarray]:
    """Extract wavelengths from HDR file metadata."""
    try:
        ...
        if hasattr(hdr, 'bands') and hasattr(hdr.bands, 'centers'):
            ...
            logger.info(f"Extracted {len(wavelengths)} wavelengths from HDR: [{wavelengths[0]:.1f}, ..., {wavelengths[-1]:.1f}] nm")
            return wavelengths
        # Try to read from metadata dict
        if hasattr(hdr, 'metadata') and 'wavelength' in hdr.metadata:
            wl = hdr.metadata['wavelength']
            wavelengths = np.array([float(w) for w in wl])
            logger.info(f"Extracted {len(wavelengths)} wavelengths from metadata: [{wavelengths[0]:.1f}, ..., {wavelengths[-1]:.1f}] nm")
            return wavelengths
    except Exception as e:
...
def preprocess_cube_for_binary_model(cube: np.ndarray, wavelengths: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Apply preprocessing for binary classification models (same as training):
    1. Filter wavelengths to [BINARY_WL_MIN, BINARY_WL_MAX] nm
    2. Apply SNV normalization per spectrum

    Args:
        cube: HSI cube of shape (H, W, C)
        wavelengths: Array of wavelength values for each band, or None

    Returns:
        X: Preprocessed feature matrix of shape (H*W, C_filtered) with SNV applied
    """
    t0 = time.perf_counter()
    H, W, C = cube.shape

    # Filter wavelengths if available
    if wavelengths is not None and len(wavelengths) == C:
        band_mask = (wavelengths >= BINARY_WL_MIN) & (wavelengths <= BINARY_WL_MAX)
        band_indices = np.where(band_mask)[0]
        cube_filtered = cube[:, :, band_indices]
        logger.info(f"Wavelength filtering: {C} -> {len(band_indices)} bands [{BINARY_WL_MIN}-{BINARY_WL_MAX} nm]")
    else:
        cube_filtered = cube
        logger.warning("No wavelength info available, using all bands for binary model")

    # Reshape to (N, C)
    H, W, C_filtered = cube_filtered.shape
    X_raw = cube_filtered.reshape(-1, C_filtered).astype(np.float32)
    ...
    logger.info(f"Binary model preprocessing: SNV applied to {X.shape[0]} pixels, {X.shape[1]} features (%.2fs)",
                time.perf_counter() - t0)
    return X


def per_pixel_probs(cube: np.ndarray, lda: object, scaler: Optional[object], pos_idx: int,
                    apply_binary_preprocessing: bool = False, wavelengths: Optional[np.ndarray] = None) -> np.ndarray:
    """Compute per-pixel crack probabilities.

    Args:
        cube: HSI cube of shape (H, W, C)
        lda: Trained model with predict_proba
        scaler: Optional scaler for feature transformation
        pos_idx: Index of positive class in probability output
        apply_binary_preprocessing: If True, apply SNV + wavelength filtering (for binary balanced models)
        wavelengths: Wavelength values for each band (needed for binary preprocessing)

    Returns:
        prob_map: 2D array of shape (H, W) with per-pixel probabilities
    """
    t0 = time.perf_counter()
    H, W, C = cube.shape

    # Use binary preprocessing if requested (for balanced binary models)
    if apply_binary_preprocessing:
        logger.info("Applying binary model preprocessing (SNV + wavelength filtering)")
        X = preprocess_cube_for_binary_model(cube, wavelengths)

        # Predict probabilities directly (binary models include their own scaler in pipeline)
        try:
            ...
            logger.error(f"Error in per_pixel_probs with binary preprocessing: {e}")
            probs = np.zeros((H, W), dtype=np.float32)

        logger.info("Per-pixel probs (binary): time=%.2fs | min=%.4f max=%.4f mean=%.4f",
                    time.perf_counter()-t0, float(probs.min()), float(probs.max()), float(probs.mean()))
        return probs

    # Original preprocessing path (for non-binary models)
