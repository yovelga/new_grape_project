...
print('\n' + '='*70)
print('FEATURE SELECTION EXPERIMENT: Testing 1-20 features using SFS')
print('='*70)
...
for n_features in tqdm(range(1, 21), desc="Testing feature counts"):
    ...
    if accuracy > best_accuracy:
        ...
        best_model = (sfs, lda)  # Store both SFS and LDA for the best model
...
with open(BEST_MODEL_REPORT, 'w') as f:
    f.write('='*70 + '\n')
    f.write('BEST MODEL REPORT - Feature Selection Experiment (SFS)\n')
    f.write('='*70 + '\n\n')
...
plt.title('Feature Selection Performance: LDA with SFS (Forward)', fontsize=14, fontweight='bold')
