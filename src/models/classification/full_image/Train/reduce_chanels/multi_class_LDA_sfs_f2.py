...
from sklearn.metrics import (classification_report, accuracy_score, f1_score, confusion_matrix, fbeta_score,
...
RESULTS_FOLDER = r'results_multi_class_reduce_CRACK_F2'
MODEL_OUTPUT_PATH = os.path.join(RESULTS_FOLDER, 'lda_model_best_crack_f2.joblib')
FEATURE_LOG_CSV = os.path.join(RESULTS_FOLDER, 'feature_selection_log_crack_f2.csv')
BEST_MODEL_REPORT = os.path.join(RESULTS_FOLDER, 'best_model_report_crack_f2.txt')
ACCURACY_PLOT = os.path.join(RESULTS_FOLDER, 'crack_f2_plot.png')
...
SELECTION_SAMPLE_SIZE = 50000  # Use full training set (no subsampling)
print('\n' + '='*70)
...
print('\n' + '='*70)
print('COMPREHENSIVE BENCHMARKING: Optimizing for CRACK F2-Score')
print('='*70)
print('Testing 1-20 features using SFS (Forward)')
print('Strategy: SFS on subset → Train & Evaluate on FULL data')
print('Best Model Selection: Maximum F2-Score for CRACK class (beta=2)')
...
for n_features in tqdm(range(1, 21), desc="Testing feature counts"):
    ...
    print(f'  SFS will perform forward selection with 3-fold CV...')
    ...
    # Track best model (based on CRACK F2-Score)
    if crack_f2 > best_crack_f2:
        best_crack_f2 = crack_f2
        ...
        print(f'  🌟 NEW BEST MODEL! CRACK F2={crack_f2:.4f}')
...
print('='*90)
print(f"{'k':<4} {'F1':<7} {'Acc':<7} {'Prec':<7} {'Rec':<7} {'AUC':<7} | {'C_F2':<7} {'C_F1':<7} {'C_Rec':<7}")
print('-'*90)
...
print('\n' + '='*70)
print('BEST MODEL FOUND (Optimized for CRACK Detection)')
print('='*70)
print(f'🎯 Best CRACK F2-Score: {best_crack_f2:.4f} ⭐')
...
print(f'  F2-Score:           {best_crack_f2:.4f} ⭐')
...
model_package = {
    ...
    'crack_f2_score': best_crack_f2,
    'accuracy': best_accuracy,
    'f1_score_weighted': best_f1_score,
    'feature_columns': feature_columns,
    'optimization_metric': 'CRACK F2-Score (beta=2)'
}
joblib.dump(model_package, MODEL_OUTPUT_PATH)
print(f'\n✓ Best LDA model (CRACK F2-optimized) saved to: {MODEL_OUTPUT_PATH}')

# Save detailed report for the best model
with open(BEST_MODEL_REPORT, 'w') as f:
    f.write('='*70 + '\n')
    f.write('BEST MODEL REPORT - CRACK F2-Score Optimization\n')
    f.write('='*70 + '\n\n')
    ...
    f.write(f'Optimization metric: F2-Score for CRACK class (beta=2)\n')
    f.write(f'Note: Beta=2 prioritizes Recall over Precision\n')
    f.write(f'Test set size: {len(best_y_test)} samples\n\n')

    f.write('='*70 + '\n')
    f.write('BEST MODEL DETAILS\n')
    f.write('='*70 + '\n')
    f.write(f'*** This model was selected to MAXIMIZE CRACK DETECTION ***\n\n')
    f.write(f'Optimal number of features: {best_n_features}\n')
    f.write(f'CRACK F2-Score: {best_crack_f2:.4f} ⭐\n')
...
# Create visualization: Number of Features vs CRACK F2-Score
...
# Plot CRACK F2 as primary metric (bold line)
plt.plot(num_features_list, crack_f2_list, marker='D', linewidth=3, markersize=10,
         label='CRACK F2-Score (β=2)', color='#d62728', zorder=3)
plt.plot(num_features_list, accuracy_list, marker='o', linewidth=2, markersize=8,
         label='Accuracy', alpha=0.6)
plt.plot(num_features_list, f1_list, marker='s', linewidth=2, markersize=8,
         label='F1-Score (weighted)', alpha=0.6)

# Mark the best point (based on CRACK F2)
plt.scatter([best_n_features], [best_crack_f2], color='gold', s=300, marker='*',
            edgecolors='black', linewidths=2,
            label=f'Best (k={best_n_features}, F2={best_crack_f2:.4f})', zorder=5)

plt.xlabel('Number of Features', fontsize=12)
plt.ylabel('Score', fontsize=12)
plt.title('CRACK Detection Optimization: F2-Score vs Number of Features', fontsize=14, fontweight='bold')
plt.grid(True, alpha=0.3)
plt.legend(fontsize=10, loc='best')
plt.xticks(range(1, 21))
plt.ylim([min(min(crack_f2_list), min(accuracy_list), min(f1_list)) - 0.02,
          max(max(crack_f2_list), max(accuracy_list), max(f1_list)) + 0.02])
plt.tight_layout()
...
ax.plot(num_features_list, f1_weighted_list,
        ...
        label='Global: F1-Weighted', color='#2ca02c', zorder=3)

ax.plot(num_features_list, recall_weighted_list,
        marker='^', linewidth=2.5, markersize=7,
        label='Global: Recall', color='#1f77b4', zorder=3)
...
# Mark the best F2 point
ax.scatter([best_n_features], [best_crack_f2],
          color='gold', s=400, marker='*',
          edgecolors='black', linewidths=2.5,
          label=f'Best F2 (k={best_n_features})', zorder=5)

# Formatting
ax.set_xlabel('Number of Features (k)', fontsize=13, fontweight='bold')
ax.set_ylabel('Score', fontsize=13, fontweight='bold')
ax.set_title('Comprehensive Benchmarking: Global & CRACK Metrics (CRACK F2 Optimization)', 
...
textstr += f'CRACK F2: {best_crack_f2:.4f}\n'
textstr += f'CRACK Recall: {best_all_metrics.get("crack_recall", 0):.4f}\n'
...
print(f'  4. {ACCURACY_PLOT} - CRACK F2 performance visualization')
