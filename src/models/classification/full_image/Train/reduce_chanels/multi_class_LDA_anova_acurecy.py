...
# ANOVA is very fast, so we use the full training set
print('\n' + '='*70)
print('DATASET INFO')
print('='*70)
print(f'Full training set size: {len(X_train)} samples')
print(f'Full test set size: {len(X_test)} samples')
print(f'Using FULL training set for ANOVA feature selection (very fast!)')

# Use full training set for ANOVA
X_train_sub = X_train
y_train_sub = y_train

# Feature Selection Experiment: Find optimal number of features (1-20)
print('\n' + '='*70)
print('FEATURE SELECTION EXPERIMENT: Testing 1-20 features using ANOVA')
print('='*70)
print('Strategy: ANOVA (SelectKBest with f_classif) → Train & Evaluate LDA')
print('='*70 + '\n')
...
for n_features in tqdm(range(1, 21), desc="Testing feature counts"):
    ...
    # Perform ANOVA feature selection using SelectKBest
    print(f'  Running ANOVA (SelectKBest) on {len(X_train_sub)} samples...')
    selector = SelectKBest(score_func=f_classif, k=n_features)
    selector.fit(X_train_sub, y_train_sub)
    print(f'  ✓ ANOVA feature selection completed!')
    ...
    # Print progress
    print(f'  ✓ RESULT: k={n_features}, Accuracy={accuracy:.4f}, F1={f1_weighted:.4f}')

    # Track best model (based on accuracy)
    if accuracy > best_accuracy:
        best_accuracy = accuracy
        best_n_features = n_features
        best_model = (selector, lda)  # Store both ANOVA selector and LDA for the best model
...
# Save the best model (both ANOVA selector and LDA)
model_package = {
    'anova_selector': best_model[0],
    'lda': best_model[1],
...
plt.title('Feature Selection Performance: LDA with ANOVA (f_classif)', fontsize=14, fontweight='bold')
...
print(f'  X_selected = model_pkg["anova_selector"].transform(X)')
