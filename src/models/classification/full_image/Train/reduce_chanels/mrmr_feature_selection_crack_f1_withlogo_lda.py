...
def prepare_logo_splits(
    ...
    Class imbalance handled via equal priors in LDA training.
    ...
    print(f"✓ Class imbalance handled via equal priors in LDA training")
    ...
    df_cr = df[df['label'].isin(CR_CLASSES)].copy()
    df_other = df[df['label'].isin(OTHER_CLASSES)].copy()
    ...
    for group in all_cr_groups:
        ...
        if len(group_classes) == 2 and 'CRACK' in group_classes and 'REGULAR' in group_classes:
            logo_validation_groups.append(group)
            crack_count = len(group_data[group_data['label'] == 'CRACK'])
            regular_count = len(group_data[group_data['label'] == 'REGULAR'])
            print(f"    ✓ {group}: BOTH classes (CRACK={crack_count}, REGULAR={regular_count}) → Validation fold")
        else:
            always_train_groups.append(group)
            class_list = ', '.join([f"{cls}={len(group_data[group_data['label'] == cls])}" for cls in group_classes])
    ...
    for label, count in df_cr['label'].value_counts().items():
        print(f"    {label}: {count} ({count/len(df_cr)*100:.2f}%)")
    ...
    if len(df_other) > 0:
        ...
        for label, count in train_other['label'].value_counts().items():
            print(f"    {label}: {count} ({count/len(train_other)*100:.2f}%)")

        print(f"\n  Val_other class distribution:")
        for label, count in val_other['label'].value_counts().items():
            print(f"    {label}: {count} ({count/len(val_other)*100:.2f}%)")

        print(f"\n  Test_other class distribution:")
        for label, count in test_other['label'].value_counts().items():
            print(f"    {label}: {count} ({count/len(test_other)*100:.2f}%)")
    else:
...
def compute_mrmr_ranking(X_train_sub: pd.DataFrame, y_train_sub: pd.Series,
    ...
    max_per_class = np.inf  # Use all samples per class
...
def evaluate_feature_subset(k: int, ranked_indices: List[int],
    ...
    Uses equal priors to handle class imbalance (no oversampling or sample weights).
    ...
    # Calculate equal priors for class balancing
    unique_labels = sorted(y_train_sub.unique())
    n_classes = len(unique_labels)
    equal_priors = [1.0 / n_classes] * n_classes

    # Train LDA with equal priors for class balancing
    lda_model = LinearDiscriminantAnalysis(priors=equal_priors)
...
def evaluate_best_mrmr_model_logo(
    ...
    print(f"\n✓ Using equal priors for class balancing (no oversampling or sample weights)")
    ...
    # Train final model with equal priors for class balancing
    print(f"\n✓ Training final LDA model with equal priors for class balancing...")
    final_model = LinearDiscriminantAnalysis(priors=equal_priors)
    final_model.fit(X_train_subset, y_train_final)
...
def evaluate_best_mrmr_model(best_k: int, ranked_indices: List[int],
    ...
    # Calculate equal priors for class balancing
    unique_labels = sorted(y_train_balanced.unique())
    n_classes = len(unique_labels)
    equal_priors = [1.0 / n_classes] * n_classes

    # Train final model on full balanced training set with equal priors
    final_model = LinearDiscriminantAnalysis(priors=equal_priors)
...
def main():
    """Main execution pipeline with hybrid LOGO strategy (filtered validation groups, equal priors)."""
    ...
    print(" NO OVERSAMPLING - Class imbalance via equal priors ")
    print("="*80)
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"  - NO OVERSAMPLING: Class imbalance handled via equal priors\n")
    ...
    print("  - NO OVERSAMPLING: Equal priors handle class imbalance")
    print("="*80 + "\n")
