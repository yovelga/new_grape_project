...
DEFAULT_DATASET_CSV = r"C:\Users\yovel\Desktop\Grape_Project\src\preprocessing\prepare_dataset_for_full_image_classification\hole_image\late_detection_row_2_test\test_dataset_row2.csv"
    ...
    def run(self):
        ...
        try:
            ...
            if len(results) > 0:
                ...
                self.log(f"  • F2 Score:   {best.get('f2_score', 0.0):.4f}  ({best.get('f2_score', 0.0)*100:.2f}%)")
                self.log(f"  • ROC AUC:    {best['roc_auc']:.4f}  ({best['roc_auc']*100:.2f}%)")
                self.log(f"  • PR AUC:     {best.get('pr_auc', 0.0):.4f}  ({best.get('pr_auc', 0.0)*100:.2f}%)")

                # Show top 5 configurations
                self.log("")
                self.log("📋 Top 5 Configurations:")
                self.log("    Rank | Accuracy | Precision | Recall | F1     | F2     | ROC-AUC | PR-AUC | Parameters")
                self.log("    " + "-"*100)
                for i in range(min(5, len(results))):
                    row = results.iloc[i]
                    params_str = f"prob={row['prob_thr']:.3f}, blob={int(row['min_blob_size'])}, glob={row['global_threshold']:.3f}"
                    self.log(f"    #{i+1:2d}  | {row['accuracy']:.4f}   | {row['precision']:.4f}    | {row['recall']:.4f} | "
                            f"{row['f1_score']:.4f} | {row.get('f2_score', 0.0):.4f} | {row['roc_auc']:.4f}  | "
                            f"{row.get('pr_auc', 0.0):.4f}  | {params_str}")
                ...
                if len(prob_maps_test) > 0:
                    ...
                    self.log(f"  • F2 Score:   {test_metrics.get('f2_score', 0.0):.4f}  ({test_metrics.get('f2_score', 0.0)*100:.2f}%)")
                    self.log(f"  • ROC AUC:    {test_metrics['roc_auc']:.4f}  ({test_metrics['roc_auc']*100:.2f}%)")
                    self.log(f"  • PR AUC:     {test_metrics.get('pr_auc', 0.0):.4f}  ({test_metrics.get('pr_auc', 0.0)*100:.2f}%)")
...
class OptunaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            prob_maps_dict, labels_dict = load_probability_maps_from_results(df, row_filter=None)
            if len(prob_maps_dict) == 0:
            ...
            def _optuna_callback(study, trial):
                ...
                pr_auc = trial.user_attrs.get("pr_auc", 0.0)
                ...
                metrics_str = f"ACC={acc:.4f} | PREC={prec:.4f} | REC={rec:.4f} | F1={f1:.4f} | F2={f2:.4f} | ROC-AUC={auc:.4f} | PR-AUC={pr_auc:.4f}"
                # Log trial number, parameters and metrics on a single line
                self.log(f"Trial {cur}/{total}: {metrics_str} | Params: {params_str}")
            ...
            def objective(trial: optuna.Trial) -> float:
                # Optimize these 6 parameters
                prob_thr = trial.suggest_float("prob_thr", 0.5, 1.0)
                min_blob_size = trial.suggest_int("min_blob_size", 0, 1000)  # Controls number of detected patches/blobs
                patch_size = trial.suggest_categorical("patch_size", [4,8,16,32,48,64,128,256,520])
                patch_pixel_ratio = trial.suggest_float("patch_pixel_ratio", 0.01, 1.0)
                global_threshold = trial.suggest_float("global_threshold", 0.001, 0.5)
                morph_size = trial.suggest_int("morph_size", 0, 20)  # Morphological closing kernel size (0=disabled)


                # Disable other parameters (set to None)
            ...
            pr_auc = best.user_attrs.get("pr_auc", 0.0)
...
class HSILateDetectionViewer(QMainWindow):
    def __init__(self):
        ...
        self.last_grid_stats: List[Dict] = []  # Store grid/patch statistics
        self.show_patches_on_cnn = False  # Toggle for showing patches on CNN panel
    ...
    def _build_ui(self):
        ...
        # Show patches on SAM panel toggle
        self.show_patches_on_cnn_chk = QCheckBox("Show Patches on SAM Panel")
        self.show_patches_on_cnn_chk.setChecked(False)  # Default: Off
        self.show_patches_on_cnn_chk.setToolTip(
            "When checked: Show patch grid overlay on SAM panel (Panel 4)\n"
            "Patches are colored by crack density:\n"
            "Red = High crack density (>10%)\n"
            "Orange = Medium crack density (5-10%)\n"
            "Yellow = Low crack density (1-5%)"
        )
        self.show_patches_on_cnn_chk.stateChanged.connect(self._on_show_patches_changed)
        actions_row.addWidget(self.show_patches_on_cnn_chk)
        ...
        self.optuna_trials_spin.setRange(10, 10000)
    ...
    def _on_show_patches_changed(self, state):
        """Handle show patches checkbox state change - refresh Panel 4 (SAM)."""
        was_enabled = self.show_patches_on_cnn
        self.show_patches_on_cnn = (state == Qt.Checked)
        logger.info("🔄 Checkbox toggled: %s → %s (state=%d, Qt.Checked=%d)",
                   was_enabled, self.show_patches_on_cnn, state, Qt.Checked)

        # Refresh Panel 4 if we have data available
        if hasattr(self, 'last_detection_mask') and self.last_detection_mask is not None:
            logger.info("  → Refreshing Panel 4 (SAM) with patches %s", "ENABLED" if self.show_patches_on_cnn else "DISABLED")
            self._refresh_panel4()
        else:
            logger.warning("  → Cannot refresh Panel 4: No detection data (run analysis first)")

    def _refresh_panel4(self):
        """Refresh Panel 4 (SAM/REFLECTANCE panel) with or without patch overlay."""
        try:
            logger.info("🔄 _refresh_panel4 called: show_patches_on_cnn=%s", self.show_patches_on_cnn)

            if self.last_detection_mask is None:
                logger.warning("Cannot refresh Panel 4: No detection data available")
                return

            hsi_h, hsi_w = self.last_detection_mask.shape
            logger.info("  → HSI dimensions: %d x %d", hsi_h, hsi_w)

            # Get base image for Panel 4 (REFLECTANCE or RGB)
            panel4_base = None
            if hasattr(self, 'reflectance_image') and self.reflectance_image is not None:
                # Use REFLECTANCE image
                panel4_base = cv2.resize(self.reflectance_image, (hsi_w, hsi_h), interpolation=cv2.INTER_LINEAR)
                logger.info("  → Using REFLECTANCE image: %s", panel4_base.shape)
            elif hasattr(self, 'rgb_image') and self.rgb_image is not None:
                # Fallback to RGB
                panel4_base = cv2.resize(self.rgb_image, (hsi_w, hsi_h), interpolation=cv2.INTER_LINEAR)
                logger.info("  → Using RGB image: %s", panel4_base.shape)

            if panel4_base is None:
                logger.warning("Panel 4 refresh: No base image available")
                return

            # Start with base as the visualization
            panel4_viz = panel4_base.copy()

            # Add cluster mask overlay if enabled (same as before)
            cluster_mask_for_panel4 = None
            if self.last_cluster_mask is not None:
                cluster_mask_for_panel4 = cv2.rotate(self.last_cluster_mask.astype('uint8'), cv2.ROTATE_90_CLOCKWISE).astype(bool)

            if self.use_cluster_filter and cluster_mask_for_panel4 is not None:
                cluster_overlay = panel4_viz.copy()
                green_color = np.array([0, 255, 0], dtype=np.uint8)  # Green in RGB
                cluster_overlay[cluster_mask_for_panel4] = green_color
                panel4_viz = cv2.addWeighted(panel4_viz, 0.7, cluster_overlay, 0.3, 0)
                logger.info("  → Added cluster mask overlay (green)")

            logger.info("  → Grid stats available: %d patches", len(self.last_grid_stats) if self.last_grid_stats else 0)

            # Apply patch overlay if enabled
            if self.show_patches_on_cnn and self.last_grid_stats:
                logger.info("✅ APPLYING PATCHES to Panel 4!")
                # For patches, we need to rotate the image to HSI orientation first (90° CCW),
                # apply patches in HSI space, then rotate back to display orientation
                # Because grid_stats are in HSI coordinate system (rows/cols)

                # Rotate image to HSI orientation (90° CCW to match HSI cube orientation)
                panel4_hsi_oriented = cv2.rotate(panel4_viz, cv2.ROTATE_90_COUNTERCLOCKWISE)
                logger.info("  → Rotated to HSI: %s", panel4_hsi_oriented.shape)

                # Convert to grayscale for overlay_on_band function
                if len(panel4_hsi_oriented.shape) == 3:
                    gray_base = cv2.cvtColor(panel4_hsi_oriented, cv2.COLOR_RGB2GRAY)
                    logger.info("  → Converted to grayscale")
                else:
                    gray_base = panel4_hsi_oriented

                # Apply patch overlay (in HSI coordinate space)
                panel4_with_patches = overlay_on_band(gray_base, self.last_grid_stats, alpha=0.4)
                logger.info("  → Overlay applied")

                # Rotate back to display orientation (90° CW)
                panel4_final = cv2.rotate(panel4_with_patches, cv2.ROTATE_90_CLOCKWISE)
                self._show_image(panel4_final, self.rgb_sam_label)
                logger.info("✅ Panel 4 refreshed: Showing patches overlay with %d grid cells", len(self.last_grid_stats))
            else:
                logger.info("❌ NOT applying patches (enabled=%s, has_stats=%s)",
                           self.show_patches_on_cnn, bool(self.last_grid_stats))
                # No patch overlay - show base image with cluster mask only
                self._show_image(panel4_viz, self.rgb_sam_label)
                logger.info("Panel 4 refreshed: Showing base image (patches disabled)")

        except Exception as e:
            logger.exception("Failed to refresh Panel 4: %s", e)
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            # Panel 3: HSI Detection (AFTER filters) - show filtered detection
            detection_after_overlay = colorize_binary_mask(band_img, self.last_detection_mask)
            ...
            # Calculate grid stats (for patch visualization and saving results)
            grid_stats = analyze_grid(prob_map, self.cell_size, self.pix_thr)
            self.last_grid_stats = grid_stats  # Store for later use
            logger.info("🔍 Grid stats computed: %d patches | show_patches_on_cnn=%s",
                       len(grid_stats), self.show_patches_on_cnn)

            # Panel 4: REFLECTANCE image with optional Cluster Mask AND Patches
            # Use REFLECTANCE_*.png from HS/results for SAM visualization
            if hasattr(self, 'reflectance_image') and self.reflectance_image is not None:
                # Resize REFLECTANCE to match HSI dimensions for proper alignment
                hsi_h, hsi_w = self.last_detection_mask.shape
                ...
                # Apply patch overlay if enabled
                logger.info("🎨 Panel 4 rendering: show_patches_on_cnn=%s, grid_stats count=%d",
                           self.show_patches_on_cnn, len(grid_stats) if grid_stats else 0)
                if self.show_patches_on_cnn and grid_stats:
                    logger.info("✅ APPLYING PATCHES to Panel 4!")
                    # For patches, rotate to HSI orientation, apply patches, rotate back
                    panel4_hsi_oriented = cv2.rotate(panel4_viz, cv2.ROTATE_90_COUNTERCLOCKWISE)
                    logger.info("  → Rotated to HSI orientation: %s", panel4_hsi_oriented.shape)

                    # Convert to grayscale for overlay_on_band function
                    if len(panel4_hsi_oriented.shape) == 3:
                        gray_base = cv2.cvtColor(panel4_hsi_oriented, cv2.COLOR_RGB2GRAY)
                        logger.info("  → Converted to grayscale: %s", gray_base.shape)
                    else:
                        gray_base = panel4_hsi_oriented

                    # Apply patch overlay (in HSI coordinate space)
                    logger.info("  → Calling overlay_on_band with %d patches", len(grid_stats))
                    panel4_with_patches = overlay_on_band(gray_base, grid_stats, alpha=0.4)
                    logger.info("  → Overlay complete: %s", panel4_with_patches.shape)

                    # Rotate back to display orientation (90° CW)
                    panel4_final = cv2.rotate(panel4_with_patches, cv2.ROTATE_90_CLOCKWISE)
                    logger.info("  → Rotated back to display: %s", panel4_final.shape)
                    self._show_image(panel4_final, self.rgb_sam_label)
                    logger.info("✅ Panel 4: Showing patches overlay with %d grid cells", len(grid_stats))
                else:
                    logger.info("❌ NOT applying patches to Panel 4 (show_patches_on_cnn=%s, grid_stats=%s)",
                               self.show_patches_on_cnn, 'exists' if grid_stats else 'None')
                    # No patch overlay - show base image with cluster mask only
                    self._show_image(panel4_viz, self.rgb_sam_label)
                ...
                # Apply patch overlay if enabled
                if self.show_patches_on_cnn and grid_stats:
                    logger.info("✅ APPLYING PATCHES to Panel 4 (RGB fallback)!")
                    panel4_hsi_oriented = cv2.rotate(panel4_viz, cv2.ROTATE_90_COUNTERCLOCKWISE)
                    if len(panel4_hsi_oriented.shape) == 3:
                        gray_base = cv2.cvtColor(panel4_hsi_oriented, cv2.COLOR_RGB2GRAY)
                    else:
            ...
            # NO patches on Panel 5 - just plain image
            panel5_base = None
            if hasattr(self, 'cnn_candidates_image') and self.cnn_candidates_image is not None:
                # If CNN debug has been run, use the result with bboxes as base
                panel5_base = self.cnn_candidates_image.copy()
                logger.info("Panel 5: Using CNN candidates image")
            else:
                # Placeholder: show SAME image as Panel 4 (REFLECTANCE or RGB)
                if hasattr(self, 'reflectance_image') and self.reflectance_image is not None:
                    # Use REFLECTANCE image (same as Panel 4)
                    hsi_h, hsi_w = self.last_detection_mask.shape
                    panel5_base = cv2.resize(self.reflectance_image, (hsi_w, hsi_h), interpolation=cv2.INTER_LINEAR)
                    logger.info("Panel 5: Using REFLECTANCE image")
                elif hasattr(self, 'rgb_image') and self.rgb_image is not None:
                    # Fallback to RGB if REFLECTANCE not available
                    hsi_h, hsi_w = self.last_detection_mask.shape
                    panel5_base = cv2.resize(self.rgb_image, (hsi_w, hsi_h), interpolation=cv2.INTER_LINEAR)
                    logger.info("Panel 5: Using RGB image")

            # Show Panel 5 (no patches, just base image)
            if panel5_base is not None:
                self._show_image(panel5_base, self.cnn_candidates_label)
            else:
                logger.warning("Panel 5: No base image available")

            # Panel 6: RGB Image Only - show plain RGB image
            if hasattr(self, 'rgb_image') and self.rgb_image is not None:
                self._show_image(self.rgb_image, self.rgb_label)

            # Grid stats already calculated above (stored in self.last_grid_stats)
            ...
            self._save_analysis_results(prob_map, self.last_grid_stats, class_name, blob_count)
