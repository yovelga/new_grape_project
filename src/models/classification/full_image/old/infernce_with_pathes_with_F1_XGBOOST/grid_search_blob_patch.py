...
def load_probability_maps_from_results(results_df: pd.DataFrame,
                                      row_filter: Optional[int] = 1) -> Tuple[Dict, Dict]:
    """
    ...
    Args:
        ...
        row_filter: Filter by row (1=dev, 2=test, None=all rows)
    ...
    if row_filter is not None:
        ...
        # Load all rows if row_filter is None
        filter_conditions = pd.Series([True] * len(results_df), index=results_df.index)
